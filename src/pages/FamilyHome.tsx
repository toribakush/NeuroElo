  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <main className="px-6 space-y-4">
        ...
        {/* --- VISUAL DE BLOCOS (HEATMAP SIMPLIFICADO) --- */}
        {insights && (
                ...
                {DAYS_OF_WEEK.map((day, dIdx) => (
                  <div key={dIdx} className="flex flex-col gap-1 items-center">
                    {[0, 1, 2, 3].map((block) => {
                      // Soma 6h por bloco
                      let count = 0;
                      for(let h=0; h<6; h++) count += insights.matrix[dIdx][(block*6)+h];
                      return (
                        <div 
                          key={block}
...
export default FamilyHome;import React, { useState, useEffect, useMemo } from 'react';
import { useNavigate } from 'react-router-dom';
import { useQuery, useQueryClient } from '@tanstack/react-query'; 
import { Plus, Calendar, LogOut, Copy, Check, Loader2, Sun, Moon, Sunrise, AlertCircle, Activity } from 'lucide-react';
import { Button } from '@/components/ui/button';
